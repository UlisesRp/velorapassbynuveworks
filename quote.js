...
async function loadQuote(){
  ...
  const q={id:row.id,folio:row.code,publicToken:row.public_token,...(row.payload||{}),total:Number(row.total||0),createdAt:row.created_at,viewedAt:row.viewed_at,status:row.status};
